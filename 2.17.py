...
def CalEout(x_dataset,y_noiseset,theta2,s):
    err=0
    for i in range(0,20):
        if np.sign(x_dataset[i]-theta2)*y_noiseset[i]*s<0:
            err+=1
    errRate=err/20.0
    return errRate
...
while i<5000:
    ...
    if minEinIndex%2==1:
        ...
        s=1
    x_data2=ProduceXData()
    y_noise2=ProduceYNoise(x_data2)
    Eout=CalEout(x_data2,y_noise2,mtheta,s)
    totalEout+=Eout
    i+=1
print totalEout/5000.0
